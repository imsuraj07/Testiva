...
import { HiOutlineViewGrid, HiOutlineClipboardCheck, HiOutlineChartBar, HiOutlineLogout, HiOutlineMenu, HiOutlineX, HiOutlinePlay } from 'react-icons/hi';
import toast from 'react-hot-toast';

const studentLinks = [
  { path: '/student/dashboard', label: 'Dashboard', icon: HiOutlineViewGrid },
  { path: '/student/give-test', label: 'Give Test', icon: HiOutlinePlay },
  { path: '/student/results', label: 'My Results', icon: HiOutlineChartBar },
];

export default function StudentLayout({ children, user, onLogout }) {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${
                    isActive
                      ? 'bg-gradient-to-r from-secondary/20 to-secondary/5 text-secondary border-l-2 border-secondary'
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 border-b border-border flex items-center px-6 bg-surface/50 backdrop-blur-xl">
          ...
          <h1 className="text-lg font-semibold text-white">
            {studentLinks.find(l => l.path === location.pathname)?.label || 'Student Portal'}
          </h1>
        </header>
